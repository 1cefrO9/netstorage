...
void Book::createDir() {
  ...
  if (!strNewDir.isEmpty()) {
    if (strNewDir.size() > 32) {
      ...
      strncpy(pdu->caData + 32, strNewDir.toStdString().c_str(),
              strNewDir.size());
...
void Book::enterDir(const QModelIndex &index) {
  ...
  m_strEnterDir = strDirName;  // 记录即将进入的目录
  QString strCurPath = TcpClient::getInstance().curPath();
  PDU *pdu = mkPDU(strCurPath.size() + 1);
...
void Book::uploadFile() {
  ...
  if (!m_strUploadFilePath.isEmpty()) {
    ...
    memcpy(pdu->caMsg, strCurPath.toStdString().c_str(), strCurPath.size());
    sprintf(pdu->caData, "%s %lld", strFileName.toStdString().c_str(),
            fileSize);
    TcpClient::getInstance().getTcpSocket().write((char *)pdu, pdu->uiPDULen);
    free(pdu);
    pdu = NULL;
...
// 返回下载状态
bool Book::getDownloadStatus() { return m_bDownload; }

// 返回保存文件的路径
QString Book::getSaveFilePath() { return m_strSaveFilePath; }

QString Book::getShareFileName() { return m_strShareFileName; }
...
void Book::selectDestDir() {
  ...
  if (NULL != pCurItem) {
    ...
    memcpy((char *)(pdu->caMsg) + srcLen + 1,
           m_strDestDir.toStdString().c_str(), destLen);
    TcpClient::getInstance().getTcpSocket().write((char *)pdu, pdu->uiPDULen);
    free(pdu);
    pdu = NULL;
  } else {
