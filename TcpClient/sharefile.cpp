...
// 取消
void ShareFile::cancelShare() { hide(); }
...
void ShareFile::okShare() {
  ...
  QString strShareFileName =
      OperateWidget::getInstance().getBook()->getShareFileName();
  ...
  for (int i = 0; i < cbList.size(); i++) {
    if (cbList[i]->isChecked()) {
      memcpy((char *)(pdu->caMsg) + j * 32,
             cbList[i]->text().toStdString().c_str(), cbList[i]->text().size());
      j++;
    }
  }
  memcpy((char *)(pdu->caMsg) + num * 32, strPath.toStdString().c_str(),
         strPath.size());
