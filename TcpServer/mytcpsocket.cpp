  ...
  if (!m_bUpload) {
    ...
    switch (pdu->uiMsgType) {
      ...
      case ENUM_MSG_TYPE_GROUP_CHAT_REQUEST: {
        char caName[32] = {'\0'};
        strncpy(caName, pdu->caData, 32);
        QStringList onlineFriend =
            OpeDB::getInstance().handleFlushFriend(caName);
        QString temp;
      ...
      case ENUM_MSG_TYPE_FLUSH_FILE_REQUEST: {
        ...
        for (int i = 0; i < fileInfoList.size(); i++) {
          // 由于指针类型是 FileInfo*，+ i 会自动按 sizeof(FileInfo)
          // 步进，无需手动计算偏移量。
      ...
      case ENUM_MSG_TYPE_SHARE_FILE_REQUEST: {
        ...
        memcpy(respdu->caMsg, (char *)(pdu->caMsg) + size,
               pdu->uiMsgLen - size);
      ...
      case ENUM_MSG_TYPE_SHARE_FILE_NOTE_RESPOND: {
        ...
        QString strFileName =
            strShareFilePath.right(strShareFilePath.size() - index - 1);
